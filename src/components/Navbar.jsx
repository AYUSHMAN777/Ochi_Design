...
function Navbar() {
    return (
        <div className="fixed w-full py-6 px-4 z-[999] flex justify-between items-center font-['Neue_Montreal'] ">
            {/* The value 999 is enclosed within square brackets to indicate that it's a custom or specific z-index value, rather than one of the predefined values provided by Tailwind CSS. */}
            <div className="logo px-7 cursor-pointer"><svg width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div  className='links flex gap-10 '>
                {["Services", "Our Work ", "About Us ", "Insights", "Contact Us"].map((item, index) => (
                    <motion.a id='navbar'  key={index}  className={` cursor-pointer   text-lg capitalize font-light ${index === 4 && "ml-32"}`}>{item}</motion.a>
                    // By enclosing {item} within curly braces, you're telling React to evaluate item as a JavaScript expression and insert its value dynamically into the JSX markup. 
                ))}
            </div>
        </div>
